...
sigma.pinlayout = sigma.pinlayout || {};
sigma.pinlayout.PinLayout = function(graph, instance, properties) {
    ...
    this.p = {
        ...
        nodes : this.graph.nodes.filter(function(n) {
            return !n.hidden && (n.type == 'pin_connect' || n.type == 'pin');
        }),
        edges : this.graph.edges.filter(function(e) {
            return !e.source.hidden && !e.target.hidden && !e.hidden && e.type == 'pin';
        }),
        subnetworkLengths: {
        }
    };
...
sigma.publicPrototype.startPinLayout = function(properties) {
    this.pinlayout = new sigma.pinlayout.PinLayout(this._core.graph, this, properties);
    this.pinlayout.init();
    var pl = this.pinlayout;

    this.addGenerator('pinlayout', this.pinlayout.atomicGo, function() {
        if (pl.isDone()) {
            return false;
        }
        return true;
...
sigma.publicPrototype.stopPinLayout = function() {
    this.pinlayout.force_done = true;
