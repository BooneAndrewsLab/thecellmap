  ...
  */
  function translateNodes(nodeids, dispX, dispY, sceneX, sceneY, w, h, ratio) {
    ...
    nodeids.forEach(function(nodeid) {
      ...
      var moveX = node.displayX, moveY = node.displayY
      node.displayX = dispX;
      node.displayY = dispY;
      ...
      dx = Math.round(node.x - newx);
      dy = Math.round(node.y - newy);
      
      if (newx <= xMax && newx > xMin && newy > yMin && newy <= yMax) {
          if (dy) {
              translateRow(node, dy, yMax, yMin);
          }
          
          if (dx) {
              translateCol(node, dx, xMax, xMin);
          }
      }
    });
  ...
  function translateRow(node, dy, yMax, yMin) {
      if (node.y + dy > yMax || node.y + dy < yMin) return;
      
      var row = self.nodes.filter(function(n) {
          return n.y == node.y;
      });
      
      var switchRow = self.nodes.filter(function(n) {
          return n.y == node.y + dy;
      });
      ...
      switchRow.forEach(function(n) {
          n.y -= dy;
      });
  }
  
  function translateCol(node, dx, xMax, xMin) {
      if (node.x + dx > xMax || node.x + dx < xMin) return;
      
      var col = self.nodes.filter(function(n) {
          return n.x == node.x;
      });
      
      var switchCol = self.nodes.filter(function(n) {
          return n.x == node.x + dx;
      });
  ...
  this.translateCol = translateCol;
...
function Plotter(nodesCtx, edgesCtx, labelsCtx, edgelabelsCtx, hoverCtx, edgehoverCtx, graph, w, h) {
  ...
  function drawNode(node) {
    ...
    if (weight < 0.) {
        ...
        ctx.fillStyle = "rgba(0, 255, 0, " + Math.abs(weight) + ")";
    }
...
//    console.log(edge['displaySize'], self.p.edgeLabelThreshold);
    if (edge['displaySize'] >= self.p.edgeLabelThreshold || edge['forceLabel']) {
...
//    return !node['hidden'] &&
    return  (node['displayX'] + node['displaySize'] > -width / 3) &&
...
function Sigma(root, id) {
  ...
  /**
   * Last edges detected under the mouse pointer. Used by interaction listeners.
   * @private
   */
  var targetedEdges;
  ...
  this.mousecaptor.bind('drag interpolate', function(e) {
    ...
    if (self.graph.nodes.filter(self.plotter.isOnScreen).length === 0) {
        self.dispatch('nodesoffscreen');
    } else {
...
//    eventType = (e['type'] == 'mousedown' || e['type'] == 'ctrlclick' || e['type'] == 'shiftclick') ? 'downgraph' : 'upgraph';
    ...
    if (eventType == 'upgraph' || (e['type'] == 'ctrlclick' && targeted.length == 0)) {
      ...
      targetedEdges = self.graph.edges.filter(function(e) {
            return !!e['hover'];
          }).map(function(n) {
            return n.id;
          });
      ...
      self.dispatch(
        ...
        if(e['type'] == 'shiftclick') {
            ...
            if (draggedMouse) {
                ...
                selection = checkRectSelection(self.mousecaptor.mouseX, self.mousecaptor.startX, self.mousecaptor.mouseY, self.mousecaptor.startY);
            } else {
                selection = targetedEdges;
            }
            
            self.dispatch('selectionStop', {selected: selection, nodeSelect: draggedMouse});
        } else {
            if(e['type'] == 'mouseup') {
