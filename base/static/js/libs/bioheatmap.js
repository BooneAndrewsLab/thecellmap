...
org.systemsbiology.visualization.BioHeatMap = Class.create({
    ...
    initialize: function(container) {
        ...
        // Mouse listeners
        this.canvasLabel.onclick = this._getMouseXY(); // mouse event handler
        this.canvasLabel.onmousemove = this._mouseMoving(); // mouse event handler
    },
    ...
    draw: function(data, options) {
        ...
        var ctx = this.ctx;
        var canvases = [this.canvas, this.canvasLabel];
        var self = this;
        ...
        if (ctx) {
            ...
            canvases.each(function(cnv) {
                // set canvas size and draw bounding box
                cnv.style.width = self._canvasWidth;  //IE
                cnv.style.height = self._canvasHeight;//IE
                cnv.setAttribute('width', self._canvasWidth); //FF
                cnv.setAttribute('height', self._canvasHeight);//FF
            });
    ...
    _createCanvas: function(id) {
        ...
        if (!canvasObj.getContext)
            canvasObj = excanvas(canvasObj); // IE
    ...
    _setupCanvas: function() {
        ...
        this.canvas = this._createCanvas('canvas-data');
        this.canvasLabel = this._createCanvas('canvas-label');
        
        CanvasTextFunctions.enable(this.canvas.getContext("2d"));
        CanvasTextFunctions.enable(this.canvasLabel.getContext("2d"));
    },
    ...
    _drawColumLabels: function(data, colStartIndex) {
        console.log(this._fontHeight, this._cellWidth);
        if (this._fontHeight > this._cellWidth) {
            this._fontHeight = this._cellWidth;
        }
    ...
    _drawRowLabels: function(data, rowNameIndex) {
        console.log(this._fontHeight, this._cellWidth);
        if (this._fontHeight > this._cellHeight) {
            this._fontHeight = this._cellHeight;
        }
    ...
    _mouseMoving: function() {
        ...
        return function(e) {
            ...
            if (t) {
                ...
                var cell = bioHeatMap._getCellFromXY(point);
                if (cell) {
                    var row = cell.row;
                    var col = cell.col;
//                    console.log(row, col);
