...
//        canvas.fillRect(0, 0, settings['showLegendSvg'] ? width * 1.25 : width, height);
    ...
    var downloadShownData = function() {
        ...
        var annot = vizdata['annotations'].get(state.get('annotation'));

        Utils.iterVisibleNodes(function(node) {
            console.log(node);
            var node_annot = annot.get('smap')[node.id];
            var terms = annot.get('terms')[node_annot];
            if (!!terms) {
                data.push([node.label, terms.name].join('\t') + '\n');
            }
            // console.log(node_annot, terms.name);
        });
